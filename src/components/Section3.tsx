...
export default function Section3() {
  return (
    <section className="bg-white py-16 md:py-24">
      {/* Title */}
      <div className={`px-6 md:px-14 mb-10 w-full mx-auto ${CONTENT_MAX_W}`}>
      ...
      <div
        ...
        style={{
          ...
          // Match the centered max-w-2xl (42rem) content start: (100vw - 42rem)/2 + px-14, min px-6
      ...
      >
        {cards.map((card) => (
          <SpotlightCard
            key={card.title}
            className="flex-shrink-0 w-[280px] md:w-[320px] flex flex-col !p-0 overflow-hidden"
          ...
          >
            ...
            <div className="flex flex-col flex-1 px-4 pt-3 pb-4 gap-2">
              <h3 className="font-heading text-white text-[1.1rem] leading-tight">
                {card.title}
              </h3>
              <p className="font-body text-neutral-400 text-[0.85rem] leading-snug flex-1">
                {card.description}
              </p>
              <button className="mt-1 self-start bg-white text-neutral-900 rounded-full px-4 py-1.5 text-[12px] font-medium transition-opacity hover:opacity-80 active:scale-[0.97]">
                {card.cta}
              </button>
            </div>
          </SpotlightCard>
